...
The old in-repo chatbot POC (Claude tool-use loop against a mirrored Oracle
Autonomous Database) has been fully removed. The chatbot now lives in a
separate service, `lims-chatbot-service` (NestJS + LangGraph.js), which reads
live Postgres data through the Go **MCP server** (`internal/adapters/mcp/`,
`cmd/mcp-server/`) instead of a synthetic Oracle mirror. See
`docs/mcp-server-tools.md` and `docs/adr/0013-mcp-server-transport.md` for the
current architecture, and `docs/chatbot-frontend-integration.md` (updated
note at top) for the new endpoint. RBAC permission `chatbot:view`
(migration `000037`) is unchanged and is now enforced by the NestJS service.
