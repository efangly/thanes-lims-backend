...
func RegisterRoutes(r fiber.Router, h *Handler, tokens portuser.TokenService) {
	...
	docs.Patch("/:id", middleware.RequirePermission(rbac.ModuleDocument, rbac.ActionEdit), h.Update)
	docs.Delete("/:id", middleware.RequirePermission(rbac.ModuleDocument, rbac.ActionDelete), h.Delete)
	docs.Post("/:id/restore", middleware.RequirePermission(rbac.ModuleDocument, rbac.ActionDelete), h.Restore)
