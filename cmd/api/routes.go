...
func registerRoutes(v1 fiber.Router, cfg *config.Config, gdb *gorm.DB, chatbotDB *sql.DB, mirrorDB *sql.DB, fileStorage *objectstorage.Adapter, redisCache cache.Cache) *applicationpurchaseorder.AutoReorderJob {
	...
	documentHandler := httpdocument.NewHandler(
		...
		applicationdocument.NewUpdateDocumentUseCase(documentRepo, docHistoryRepo, equipmentRepo, calibrationRepo),
		applicationdocument.NewDeleteDocumentUseCase(documentRepo, docHistoryRepo),
		applicationdocument.NewRestoreDocumentUseCase(documentRepo, docHistoryRepo),
